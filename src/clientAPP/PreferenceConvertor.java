...
public class PreferenceConvertor {
	 ...
	 * Input for these methods is the Context 'context' to be able to retrieve
	 * the resources; and String 'preferenceName' which is the preference name
	...
	public void initiatePreferenceConvertor(Context context, String preference) {
		...
		int maxPreferenceIdentifier = res.getIdentifier(maxPreference, "integer",
				context.getPackageName());
		String minPreference = preference + "_min";
		int minPreferenceIdentifier = res.getIdentifier(minPreference, "integer",
				context.getPackageName());
		...
		if ((maxPreferenceIdentifier == 0) || (minPreferenceIdentifier == 0)) {
			...
			int pref1Max = res.getInteger(maxPreferenceIdentifier);
			int pref1 = pref1Max - (res.getInteger(minPreferenceIdentifier));
			adjustment = pref1Max - pref1;
		}
	}
